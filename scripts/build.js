...
esbuild
  ...
  .then(() => {
    ...
    // Copy the canonical skills from the @imagelato/skills workspace into
    // the package. The npm tarball ships them via `files: ["dist", "skills"]`
    // so `imagelato skills get <name>` always prints the guide matching the
    // installed version.
    // The canonical source only exists inside the waiterio monorepo — builds
    // from the public github.com/imagelato/cli mirror skip the copy and keep
